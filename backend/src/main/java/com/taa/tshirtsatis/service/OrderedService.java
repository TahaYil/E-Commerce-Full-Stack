...
public class OrderedService {
    ...
    public OrderedDto getOrderedById(int id) {
        ...
        if (ordered.getOrder() != null) {
            // OrderItems'ı yükle
            ordered.getOrder().getOrderItems().size();
            
            // Products'ı yükle
            ordered.getOrder().getProducts().size();
            
            // Her bir ürünün kategorilerini yükle
            ordered.getOrder().getProducts().forEach(product -> {
    ...
    public List<OrderedDto> getOrderedByUserId(int userId) {
        try {
            ...
            List<Ordered> orders = orderedRepository.findByUserId(userId);
            ...
            throw new RuntimeException("Siparişler getirilirken bir hata oluştu: " + e.getMessage(), e);
        }
    }
