...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .requestMatchers("/product/all", "/product/{id}", "/product/category/**", "/product/{id}/image").permitAll()
                .requestMatchers("/category", "/category/all", "/category/{id}").permitAll()
                .requestMatchers("/review/product/**").permitAll()
                .requestMatchers("/review/check-eligibility").permitAll()
                .requestMatchers("/review/order/**").permitAll()
                .requestMatchers(
                        "/oauth2/**", // Gerekli
                        "/login/**",  // Gerekli
                        "/v1/auth/**", // varsa kendi auth endpointlerin
                        "/error"      // hata sayfası da erişilebilir olmalı
                ).permitAll()
